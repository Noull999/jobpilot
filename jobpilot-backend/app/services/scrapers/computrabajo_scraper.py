...
class ComputrabajoScraper(BaseScraper):
    """Scraper para Computrabajo.com (Chile)"""

    def __init__(self):
        super().__init__('computrabajo')
        self.base_url = 'https://www.computrabajo.cl'
        self.headers = {
    ...
    def fetch_jobs(self, limit: int = 100, keyword: str = 'developer', location: str = None) -> List[Dict]:
        ...
        try:
            # URL de búsqueda
            search_url = f"{self.base_url}/trabajo-de-{keyword}"
            if location:
            ...
            # Buscar job listings (selectores típicos de Computrabajo)
            job_listings = soup.find_all('div', class_='box')
            ...
            self.logger.error(f"Error fetching from Computrabajo: {str(e)}")
            raise
    ...
    def _parse_job_listing(self, listing) -> Dict:
        ...
        try:
            # Intentar extraer información del listing
            ...
            url_elem = listing.find('a', class_='job-link')
    ...
    def normalize_job(self, raw_job: Dict) -> Dict:
        """Normalize raw Computrabajo job to standard format"""
        job = self._get_standard_job(raw_job)

        job.update({
            'external_id': f"computrabajo_{raw_job.get('url', '').split('/')[-1][:20]}",
            'source': 'computrabajo',
